      ...
      >
        ...
        <div className="space-y-5 mb-16 sm:mb-20">
          {personalInfo.about.map((paragraph, index) => (
            <motion.p
              ...
              className="text-base sm:text-lg text-zinc-600 dark:text-zinc-400 leading-relaxed font-light"
            >
              {paragraph}
            </motion.p>
          ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-8">
          <motion.div
            ...
            className="group relative bg-white/50 dark:bg-zinc-900/40 backdrop-blur-sm border border-zinc-200/60 dark:border-zinc-800/80 rounded-2xl sm:rounded-3xl p-6 sm:p-8 hover:shadow-xl dark:hover:shadow-black/20 hover:border-zinc-300 dark:hover:border-zinc-700 transition-all duration-300"
          >
            <div
            ...
            <div className="flex flex-wrap gap-2 sm:gap-2.5 relative z-10" role="list" aria-label="Skills and technologies">
              {toolkit.map((item) => (
                <span
                  key={item}
                  role="listitem"
                  className="px-3 sm:px-4 py-1.5 sm:py-2 bg-zinc-100/80 dark:bg-zinc-800/80 rounded-xl text-xs sm:text-sm font-medium text-zinc-700 dark:text-zinc-300 border border-zinc-200/50 dark:border-zinc-700/50 shadow-sm transition-colors hover:border-primary-500/40"
                >
                  {item}
                </span>
              ))}
